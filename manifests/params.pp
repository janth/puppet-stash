  ...
  case $::osfamily {
    /RedHat/: {
      if $::operatingsystemmajrelease == '7' {
        ...
        $service_file_location   = "/usr/lib/systemd/system/$stash::product.service"
        $service_file_template   = 'stash/stash.service.erb'
        $service_lockfile        = "/var/lock/subsys/$stash::product"
      } elsif $::operatingsystemmajrelease == '6' {
        $json_packages           = [ 'rubygem-json', 'ruby-json' ]
        $service_file_location   = "/etc/init.d/$stash::product"
      ...
      $service_lockfile        = "/var/lock/$stash::product"
    } default: {
      fail("${::operatingsystem} ${::operatingsystemmajrelease} not supported")
    }
  }
